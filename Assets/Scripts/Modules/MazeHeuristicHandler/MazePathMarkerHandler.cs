...
using Modules.Pathfinder.Extensions;
using Modules.Writers;
using UnityEngine;
...
namespace Modules.MazeHeuristicHandler
{
    public class MazePathMarkerHandler : MonoBehaviour
    {
        ...
        private bool AllIsNotZero(PathScores scores)
        {
            if (scores.GScore != 0 && scores.FScore != 0 && scores.HScore != 0)
                return true;

            return false;
        }

        private bool IsNotHValue(PathScores scores)
        {
            if (scores.HScore == 0)
                return true;
        ...
        private bool IsNotGValue(PathScores scores)
        {
            if (scores.GScore == 0)
                return true;
